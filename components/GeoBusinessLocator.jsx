  ...
  const parseCSV = (csvString) => {
    ...
    while (i < csvString.length) {
      ...
      if (char === '"') {
        ...
        // Handle newline as delimiter when not in quotes
        result.push(current.trim());
        current = "";
      } else {
        current += char;
      ...
      <div className="max-w-6xl mx-auto">
                  ...
                  <div className="flex gap-2 mb-3">
                    ...
                    <div className="flex flex-col gap-2">
                      <button
                        onClick={() => {
                          if (newKeyword.trim()) {
                            const arr = parseCSV(newKeyword);
                            // Format by showing each item on a new line, preserving quotes if they contain commas
                            const formatted = arr
                              .map((item) => {
                                // If item contains comma, wrap in quotes for display
                                return item.includes(",")
                                  ? `"${item.replace(/"/g, '""')}"`
                                  : item;
                              })
                              .join("\n");
                            setNewKeyword(formatted);
                            setError("");
                          } else {
                  ...
                  <div className="flex gap-2 mb-3">
                    ...
                    <div className="flex flex-col gap-2">
                      <button
                        onClick={() => {
                          if (newDescription.trim()) {
                            const arr = parseCSV(newDescription);
                            // Format by showing each item on a new line, preserving quotes if they contain commas
                            const formatted = arr
                              .map((item) => {
                                // If item contains comma, wrap in quotes for display
        ...
        {(isProcessing || status) && (
          <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
            <h3 className="font-semibold text-zinc-900 mb-4">Progress</h3>
            <div className="mb-2">
        ...
        {/* Results */}
        {results.length > 0 && (
          <div className="bg-white text-zinc-900 rounded-xl shadow-lg p-6">
